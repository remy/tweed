...
class db_store {
    ...
    function run($tweet) {
        if (($this->initialised && $this->connected) || $this->config['debug']) {
            $sql = sprintf('insert into ' . $this->table . ' (id, avatar, name, screen_name, message, createdDate) values (' . $tweet->id . ', "%s", "%s", "%s", "%s", "%s")', mysql_escape_string($tweet->profile_image_url), mysql_escape_string($tweet->from_user), mysql_escape_string($tweet->from_user), mysql_escape_string($tweet->text), date('c', $tweet->created_at));
